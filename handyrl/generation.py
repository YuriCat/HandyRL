...
class Generator:
    ...
    def generate(self, models, args):
        ...
        while not self.env.terminal():
            moment_keys = ['observation', 'selected_prob', 'action_mask', 'action', 'value', 'reward', 'return', 'action_length']
            ...
            for player in self.env.players():
                ...
                if player in turn_players:
                    ...
                    moment['action_mask'][player] = np.packbits(action_mask.astype(np.int8))
                    moment['action_length'][player] = len(action_mask)
