...
def make_batch(episodes, args):
    ...
    for ep in episodes:
        ...
        amask = np.array([[replace_none(np.unpackbits(m['action_mask'][player], count=amask_length).astype(np.float32) if m['action_mask'][player] is not None else None, amask_zeros + 1) * 1e32 for player in players_] for m, players_ in zip(moments, players_list)])
