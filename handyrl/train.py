...
class Learner:
    ...
    def update_model(self, model, steps):
        ...
        torch.save(model.state_dict(), self.model_path(str(self.model_epoch)))
    ...
    def update(self):
        ...
        if model_id not in self.results:
            ...
            keys = self.results_per_opponent[model_id]
        ...
        while self.worker.connection_count() > 0 or not self.shutdown_flag:
            ...
            if req == 'args':
                    ...
                    for _ in data:
                        ...
                        if args['role'] == 'g':
                            ...
                            for p in self.env.players():
                                if p in args['player']:
                                    args['model_id'][p] = str(self.model_epoch)
                                else:
                ...
                for model_id in data:
                    model = self.model
                    if model_id != str(self.model_epoch) and model_id != '0':
                        try:
