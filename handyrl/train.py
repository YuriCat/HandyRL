...
def make_batch(episodes, args):
    ...
    for ep in episodes:
        ...
        imp_zeros = map_r(ep['imperfect'][0], lambda i: np.zeros_like(i))
        ...
        imp = [ep['imperfect'][1 - player] for player in players]
        imp = bimap_r(imp_zeros, rotate(imp), lambda _, i: np.array([i]))   # (P, ..., ...) -> (..., 1, P, ...)
        ...
        if len(tmask) < batch_steps:
            ...
            #imp = map_r(imp, lambda i: np.pad(i, [(pad_len_b, pad_len_a)] + [(0, 0)] * (len(i.shape) - 1), 'constant', constant_values=0))
