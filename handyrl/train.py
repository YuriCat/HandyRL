...
def compose_losses(model, outputs, log_selected_policies, total_advantages, targets, batch, args):
    ...
    obs_ = map_r(batch['observation'], lambda o: o.unsqueeze(3).repeat(1, 1, 1, outputs['policy'].size(3), 1).flatten(0, 3))
    all_actions = torch.arange(outputs['policy'].size(3), device=outputs['policy'].device).view(1, 1, 1, -1).repeat(*batch['action'].size()).flatten()
    win = torch.clamp(targets['value'].sign(), 0, 1).mul(omasks).int()
    onehot_advantages = F.one_hot(batch['action'].squeeze(-1), -1).float() * total_advantages

    baselines = F.log_softmax(torch.zeros_like(outputs['policy']) - batch['action_mask'], -1) * batch['episode_mask']
    entropy_coef = (1 - batch['progress'].unsqueeze(-1) * (1 - args['entropy_regularization_decay'])) * args['entropy_regularization']
    entropy_bonus = torch.ones_like(onehot_advantages) * (baselines - outputs['policy'].detach()) * torch.exp(outputs['policy'].detach()) * entropy_coef

    policy_weights = torch.clamp(onehot_advantages + entropy_bonus, 0, 1).mul(tmasks)
